...
setup_kube_vip () {
    ...
    -i ~/.ssh/$SSH_KEY ./files/kube-vip-rbac.yaml \
    ubuntu@$K3S_SERVER_IP:~/kube-vip-rbac.yaml
    ...
    scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    -i ~/.ssh/$SSH_KEY ./files/kube-vip.yaml \
    ubuntu@$K3S_SERVER_IP:~/kube-vip.yaml 
...
install_metallb() {
    # install metallb
    curl -s https://raw.githubusercontent.com/metallb/metallb/v0.9.5/manifests/namespace.yaml > ./files/metallb.yaml; \
    echo '---' >> ./files/metallb.yaml; \
    curl -s https://raw.githubusercontent.com/metallb/metallb/v0.9.5/manifests/metallb.yaml >> ./files/metallb.yaml; \

    scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    -i ~/.ssh/$SSH_KEY ./files/metallb.yaml \
    ubuntu@$K3S_SERVER_IP:~/metallb.yaml 

    multipass exec $K3S_SERVER sudo mv metallb.yaml /var/lib/rancher/k3s/server/manifests/
